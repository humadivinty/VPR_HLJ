...
#define  COLOR_BLUE 00
#define  COLOR_YELLOW 01
#define  COLOR_BLACK 02
...
#ifdef GUANGXI_DLL
	strcat_s(iniFileName, FileName);
	sprintf_s(iniFileName,"\\VPR_Signalway_%d.ini",  DLL_VERSION);
#else
...
**************************************队列模式*****begin*****************************************/
    ...
    if (g_ResultCallback)
    {
        ...
        char chTemp[32] = { 0 };
        MY_SPRINTF(chTemp, "%d", m_BufferResult->iPlateColor);
